...
|分析テーマ|必要なデータ項目（Mart層のカラム）|
|--|--|
|予約行動分析|予約日、乗船日、リードタイム（日数）、予約経路（Web/窓口等）|
|顧客セグメント分析|年代（10歳刻み）、性別、居住地、累計乗船回数、平均LTV|
|需要予測・稼働分析|航路、便、曜日、祝日フラグ、定員、予約数、乗船数|

#### 詳細マップ

|ビジネス要件|分析テーマ|現場で使われる具体的なアクション（例）|
|--|--|--|
|予約・乗船分析 (売上の源泉)|予約行動分析|「予約が直前に集中しているから、早割の割引率を上げて予約を前倒しさせよう」|
|キャンペーン効果・顧客分析 (集客の効率化)|顧客セグメント分析|「夏休みに乗船した新規客が、秋にリピートしていない。秋の割引クーポンを新規客に送ろう」|
|運航効率の最適化|需要予測・稼働分析|火曜日の1便目は常に乗船率20%以下だ。この便だけ小型船に切り替えるか、減便を検討しよう」|

#### その他

- 課題の提示: 「旅客船ビジネスは、便ごとの固定費（燃料・人件費）が高いため、**乗船率（稼働）** の平準化が最優先課題」
- データ基盤の役割: 「基幹DBからデータを抽出し、**『予約行動』『顧客属性』『便別稼働』** を多角的に分析できる基盤を構築」
- 解決の鍵: 「dbtを用いて **『予約リードタイム（出港何日前に予約されたか）』** を算出することで、マーケティング担当者がキャンペーンを打つべき最適なタイミングを特定」
...
|--|--|--|--|--|--|--|  
|1便|Orange / Grape|07:00|08:40||09:20|11:00|
|2便|Grape / Orange|09:20|11:00||07:00|08:40|
|3便|Orange / Grape|11:40|13:20||14:00|15:40|
|4便|Grape / Orange|14:00|15:40||11:40|13:20|
|5便|Orange / Grape|16:20|18:00||18:40|20:20|
|6便|Grape / Orange|18:40|20:20||16:20|18:00|
|7便|Orange / Grape|21:00|22:40||23:20|01:00|
|8便|Grape / Orange|23:20|01:00||21:00|22:40|
...
- int_vessel_schedules: stg_schedules に ships, sections, routes, ports を結合
  - 目的: 「いつ、どの船が、どこからどこへ行くか」という情報を1つのワイドなテーブルにし、後の集計を楽にする。
- int_reservation_combined: stg_reservations と stg_reservation_details を結合。
  - 目的: 予約者情報と明細（乗客ごと）を紐付け。ここで reservation_date と departure_date の差分を計算し、**「予約リードタイム」**を算出する。
- int_daily_inventory_snapshots: stg_inventory をベースに、在庫消化率などを計算。
...
- 【予約・乗船分析】向け
  ...
  - 粒度: 予約明細（1チケット）ごと
  - カラム: 予約ID, スケジュールID, 船ID, 航路名, 客室クラス名, 適用料金, 予約リードタイム, 乗客区分(大人/子供), 曜日フラグ
  - 用途: 売上集計、予約の早まり・遅まり分析。
- 【キャンペーン効果・顧客分析】向け
  - dim_passengers (顧客ディメンション)
  - 粒度: 乗客（Passenger）ごと
  - カラム: 乗客ID, 累計乗船回数, 初回乗船日, 直近乗船日, よく利用する航路, 推定年代(birth_dateから計算)
  - 用途: リピーター分析、ターゲット層の特定。
- 【運航効率の最適化】向け
  - fct_voyage_performance (航海実績ファクト)
  - 粒度: スケジュール（便）ごと
  - カラム: スケジュールID, 船名, 航路, 出港日, 曜日, 定員(max_capacity), 確定予約人数, 乗船率(%), 在庫消化速度
...
- ローカルのDocker上で PostgreSQL -> dbt -> BigQuery の流れを作る。
- TerraformでGoogle Cloudの環境を構築する。
- Dagsterを導入して、Cloud Run Jobとしてデプロイ・スケジューリングする。
- GitHub ActionsでCI/CD（テストの自動実行）を組む。
