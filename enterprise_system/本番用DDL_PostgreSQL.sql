...
/* 船マスタ（基本情報）テーブル */
DROP TABLE IF EXISTS ships_raw.ships CASCADE;
CREATE TABLE IF NOT EXISTS ships_raw.ships (
...
INSERT INTO ships_raw.ships (ship_id, ship_name, length, width, gross_tonnage, service_speed, max_passenger_capacity, start_date, end_date) VALUES
...
INSERT INTO ships_raw.room_class_master VALUES
...
/* 船別客室設定テーブル */
DROP TABLE IF EXISTS ships_raw.ship_room_classes CASCADE;
CREATE TABLE IF NOT EXISTS ships_raw.ship_room_classes (
  ship_id CHAR(4)
  , room_class_id CHAR(2)
  , room_count INTEGER
  , capacity_per_room INTEGER
  ...
  , FOREIGN KEY (room_class_id, capacity_per_room) REFERENCES ships_raw.room_class_master(room_class_id, capacity_per_room)
);

INSERT INTO ships_raw.ship_room_classes (ship_id, room_class_id, room_count, capacity_per_room, total_occupancy) VALUES
 -- Apple, Banana (502名)
...
/* 港テーブル */
DROP TABLE IF EXISTS ships_raw.ports CASCADE;
CREATE TABLE IF NOT EXISTS ships_raw.ports (
  port_id CHAR(2) PRIMARY KEY
  , port_name VARCHAR(64)
);

INSERT INTO ships_raw.ports VALUES
...
/* 航路テーブル */
DROP TABLE IF EXISTS ships_raw.rooutes;
CREATE TABLE IF NOT EXISTS ships_raw.rooutes (
  route_id CHAR(3) PRIMARY KEY
  , departure_port_id CHAR(2) REFERENCES ships_raw.ports(port_id)
  , arrival_port_id CHAR(2) REFERENCES ships_raw.ports(port_id)
);

INSERT INTO ships_raw.rooutes (route_id, departure_port_id, arrival_port_id) VALUES
 ('R1', 'P1', 'P2'), ('R2', 'P1', 'P3')
;


/* 区間テーブル */
DROP TABLE IF EXISTS ships_raw.sections;
CREATE TABLE IF NOT EXISTS ships_raw.sections (
...
INSERT INTO ships_raw.sections (section_id, departure_port_id, arrival_port_id, travel_time_minutes) VALUES
...
/* 航路区間構成テーブル */
DROP TABLE IF EXISTS ships_raw.route_sections;
CREATE TABLE IF NOT EXISTS ships_raw.route_sections (
  section_id CHAR(2) REFERENCES ships_raw.sections(section_id)
  , route_id CHAR(3) REFERENCES ships_raw.rooutes(route_id)
  , PRIMARY KEY (section_id, route_id)
);

INSERT INTO ships_raw.route_sections (section_id, route_id) VALUES
...
/* 運行スケジュールテーブル */
DROP TABLE IF EXISTS ships_raw.schedule;
CREATE TABLE IF NOT EXISTS ships_raw.schedule (
  schedule_id CHAR(14)
  , route_id CHAR(3) REFERENCES ships_raw.rooutes(route_id)
  , section_id CHAR(2) REFERENCES ships_raw.sections(section_id)
  , departure_time TIMESTAMP
  , arrival_time TIMESTAMP
  , ship_id CHAR(4) REFERENCES ships_raw.ships(ship_id)
);

DELETE FROM ships_raw.schedule;
INSERT INTO ships_raw.schedule (
...
/* 予約基本情報テーブル */
DROP TABLE IF EXISTS ships_raw.reservations;
CREATE TABLE IF NOT EXISTS ships_raw.reservations (
...
/* 予約明細情報テーブル */
DROP TABLE IF EXISTS ships_raw.reservation_details;
CREATE TABLE IF NOT EXISTS ships_raw.reservation_details (
  reservation_id CHAR(12) REFERENCES ships_raw.reservations(reservation_id)
  , detail_id CHAR(3)
  , section_id CHAR(2) REFERENCES ships_raw.sections(section_id)
  , schedule_id CHAR(14) REFERENCES ships_raw.schedule(schedule_id)
  , passenger_id CHAR(8)
  ...
  , FOREIGN KEY (room_class_id) REFERENCES ships_raw.room_class_master(room_class_id)
);
...
CREATE TABLE IF NOT EXISTS ships_raw.inventry (
  ...
  , FOREIGN KEY (room_class_id) REFERENCES ships_raw.room_class_master(room_class_id)
  , FOREIGN KEY (section_id) REFERENCES ships_raw.sections(section_id)
);
...
SELECT 
  ...
  , src.total_occupancy
FROM
  ships_raw.schedule s
  INNER JOIN ships_raw.ship_room_classes src ON s.ship_id = src.ship_id
;
